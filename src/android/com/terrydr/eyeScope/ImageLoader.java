...
import com.terrydr.eyeScope.ImageSizeUtil.ImageSize;
...
public class ImageLoader {
	...
	private void init(int threadCount, Type type, Context context) {
		...
		// 注意此处要获取全Context，避免引用Activity造成资源无法释放
		mContext = context.getApplicationContext();
		mLruCache = new LruCache<String, Bitmap>(cacheMemory) {
			@Override
			protected int sizeOf(String key, Bitmap value) {
				return value.getRowBytes() * value.getHeight(); // 旧版本方
			}
		};
			...
			public void run() {
				while (true) {
					try {
						...
						// 线程池去取出个任务进行执行，当任务队列为空时，阻塞线
						Runnable runnable = getTask();
						// 使用线程池执行任
						mThreadPool.execute(runnable);
					} catch (InterruptedException e) {
						Log.e(TAG,"使用线程池执行任" ,e);
					}
				}
	...
	public String bytes2hex02(byte[] bytes) {
		...
		String tmp = null;
		for (byte b : bytes) {
			// 将每个字节与0xFF进行与运算，然后转化进制，然后助于Integer再转化为16进制
			tmp = Integer.toHexString(0xFF & b);
			if (tmp.length() == 1)// 每个字节8为，转为16进制标志进制
			{
				tmp = "0" + tmp;
			}
			sb.append(tmp);
